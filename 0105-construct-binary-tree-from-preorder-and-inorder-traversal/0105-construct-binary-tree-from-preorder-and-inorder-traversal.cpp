...
public:
    ...
    TreeNode* buildTree(vector<int>& preorder,int prestart,int preend,vector<int>& inorder,int instart,int inend,map<int,int>& mp)
    {
        ...
        int pos=inroot-instart;
        
        root->left=buildTree(preorder,prestart+1,prestart+pos,inorder,instart,inroot-1,mp);
        root->right=buildTree(preorder,prestart+pos+1,preend,inorder,inroot+1,inend,mp);
        return root;
    }
};